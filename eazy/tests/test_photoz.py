...
from .test_templates import read_template_ascii
...
@pytest.fixture
def make_fake_catalog(read_template_ascii):
    """
    Make a fake photometric catalog
    """
    os.chdir(utils.DATA_PATH)
...
def test_full_photoz(make_fake_catalog):
    """
    End-to-end test
    """
    global ez
...
def test_sps_parameters():
    ...
    assert np.allclose(z2["z_phot"], zuser, rtol=1.0e-2)
...
def test_load_products():
    ...
    new = photoz.PhotoZ(
        param_file="eazy_test.zphot.param",
    ...
    for k in ez.param.params:
        assert bool_param(ez.param.params[k]) == bool_param(new.param.params[k])
...
def test_hdf5():
    ...
    assert np.allclose(ez.lnp, new_ez.lnp, rtol=1.0e-4)
