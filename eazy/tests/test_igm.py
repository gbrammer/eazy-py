...
# More comprehensive tests in test_templates
...
def test_igm():
    ...
    assert np.allclose(igm_obj.full_IGM(z, rest_wave * (1 + z)), igm_val, rtol=1.0e-2)
    ...
    assert np.allclose(
        igm_obj.full_IGM(z, rest_wave * (1 + z)), igm_scaled, rtol=1.0e-2
    )
